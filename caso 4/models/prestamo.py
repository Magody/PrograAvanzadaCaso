...
class PrestamoManager:
    ...
    def __init__(self):
        ...
        if PrestamoManager._instance is not None:
            ...
            try:
                self._df = pd.read_csv("./data/prestamos_libros.csv")  # Lee el archivo CSV en el DataFrame
    ...
    def _procesar_concurrentemente(self, num_hilos=4):
        """
        Procesa el DataFrame en partes usando múltiples hilos.
        """
        if len(self._df) < num_hilos:
            # Ajusta el número de hilos si hay menos filas que hilos
            num_hilos = len(self._df)
        
        tamaño_parte = len(self._df) // num_hilos  # Calcula el tamaño de cada parte del DataFrame
        hilos = []  # Lista para almacenar los hilos

        for i in range(num_hilos):
            inicio = i * tamaño_parte  # Índice de inicio para la parte del DataFrame
            fin = len(self._df) if i == num_hilos - 1 else (i + 1) * tamaño_parte  # Índice final para la parte del DataFrame
            df_part = self._df.iloc[inicio:fin]  # Extrae la parte del DataFrame
